...
data_qpr = pd.DataFrame() 

try:
    
    ruta_archivo = 'Cars Datasets 2025.csv' 
    data_carros = pd.read_csv(ruta_archivo, encoding='latin-1') 
    
except FileNotFoundError:
    st.error(f"❌ Error: Archivo CSV no encontrado en la ruta: {ruta_archivo}. Para despliegue, la ruta debe ser relativa.")
...
if not data_carros.empty:
    ...
    try:
        ...
        data_carros = pd.DataFrame() 
        data_qpr = pd.DataFrame() # Vuelve a vaciar data_qpr si falla el proceso de limpieza
        
if not data_qpr.empty: # Usar data_qpr para la lógica que sigue
...
if opcion_seleccionada == "Introducción":
    ...
    # Condición de verificación para evitar el NameError
    if not data_qpr.empty: 
        ...
        st.warning("⚠️ No se pudieron cargar o procesar los datos para las métricas.")
    
elif opcion_seleccionada == "Top 10 Modelos":
    ...
    if not data_qpr.empty:
        ...
        st.warning("⚠️ No hay datos suficientes para mostrar el Top 10.")

elif opcion_seleccionada == "Comparativa por Marcas":
    ...
    if not data_qpr.empty:
        ...
        st.warning("⚠️ No hay datos suficientes para la Comparativa por Marcas.")

elif opcion_seleccionada == "Análisis por Combustible":
    ...
    if not data_qpr.empty:
        ...
        st.warning("⚠️ No hay datos suficientes para el Análisis por Combustible.")
